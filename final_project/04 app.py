...
# Load the models
production_pyfunc = mlflow.pyfunc.load_model(production_model.source)
staging_pyfunc = mlflow.pyfunc.load_model(staging_model.source)

# COMMAND ----------

# Get the station information
station_df = spark.read.format('delta').load(BRONZE_STATION_INFO_PATH).filter(col("name") == GROUP_STATION_ASSIGNMENT).toPandas()
...
weather_df = spark.read.format('delta').load(BRONZE_NYC_WEATHER_PATH).filter(col("time") >= now).limit(1).toPandas()
...
station_status_df = (
    ...
    .withColumn("last_reported_ts", from_unixtime(col("last_reported")))
    .filter(col("last_reported_ts") >= now)
    .limit(1)
    .toPandas()
)
...
forecast_df = pd.DataFrame({"ds": pd.date_range(start=now, periods=hours_to_forecast+1, freq='H')})
production_forecast = production_pyfunc.predict(forecast_df)
staging_forecast = staging_pyfunc.predict(forecast_df)

# COMMAND ----------

# Calculate the error for each model and plot the residuals
production_residuals = production_forecast['yhat'] - station_status_df['num_bikes_available']
staging_residuals = staging_forecast['yhat'] - station_status_df['num_bikes_available']

# Plot the residual plot
plt.figure(figsize=(10, 6))
plt.scatter(production_forecast['ds'], production_residuals, label='Production Model')
plt.scatter(staging_forecast['ds'], staging_residuals, label='Staging Model')
plt.xlabel('Time')
...
# Archive the current production model and promote the staging model if specified
if promote_model:
    client.transition_model_version_stage(name=model_name, version=production_model.version, stage='Archived')
    client.transition_model_version_stage(name=model_name, version=staging_model.version, stage='Production')
